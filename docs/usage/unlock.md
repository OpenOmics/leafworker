# <code>leafworker <b>unlock</b></code>
...
This part of the documentation describes options and concepts for <code>leafworker <b>unlock</b></code> sub command in more detail. With minimal configuration, the **`unlock`** sub command enables you to unlock a pipeline output directory. 
...
Unlocking leafworker pipeline output directory is fast and easy! In its most basic form, <code>leafworker <b>unlock</b></code> only has *one required input*.

## 2. Synopsis
```text
$ ./leafworker unlock [-h] --output OUTPUT
```
...
leafworker unlock --output /data/$USER/output
```