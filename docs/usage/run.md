...
Setting up the leafworker pipeline is fast and easy! In its most basic form, <code>leafworker <b>run</b></code> only has *two required inputs*.
...
./leafworker run --input .tests/*.R?.fastq.gz \
